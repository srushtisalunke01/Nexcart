  ...
  if (compareList.length === 0) {
    return (
      <div className="text-center py-20 max-w-md mx-auto space-y-6">
        ...
        <div>
          <h2 className="text-xl font-display font-extrabold text-slate-805 dark:text-white">Comparison Deck is Empty</h2>
          <p className="text-slate-400 text-xs mt-1 leading-normal">
            You can add up to 4 items side-by-side. Tap "Compare specs" on any product detail page to start analyzing models.
          </p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Comparing details for {compareList.length} items side-by-side.</p>
          </div>
        </div>
        ...
        <table className="w-full border-collapse text-left text-xs min-w-[700px]">
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
              <th className="p-5 font-bold text-slate-400 uppercase tracking-wider w-1/5">Details</th>
              {compareList.map(prod => (
                <th key={prod.id} className="p-5 w-1/5 relative">
                  ...
                  <div className="flex flex-col items-center text-center space-y-2 mt-4">
                    <img src={prod.images[0]} alt={prod.name} className="h-20 w-20 object-contain bg-white rounded-lg p-1.5" />
                    <div>
                      <h3 className="font-extrabold text-slate-800 dark:text-white leading-tight truncate max-w-[140px]">
                        {prod.name}
                      </h3>
                      <p className="text-[9px] font-bold text-slate-400 uppercase">{prod.brand}</p>
                    </div>
                  </div>
                </th>
              ))}
          ...
          <tbody>
            ...
            {/* Rating */}
            <tr className="border-b border-slate-50 dark:border-slate-900/40">
              <td className="p-5 font-bold text-slate-400 uppercase tracking-wider">Ratings</td>
              {compareList.map(prod => (
                <td key={prod.id} className="p-5 text-center">
                  ...
                  <span className="text-[9px] text-slate-400">{prod.reviewsCount} reviews</span>
                </td>
              ))}
            </tr>
            ...
            {uniqueSpecKeys.map(specKey => (
              <tr key={specKey} className="border-b border-slate-50 dark:border-slate-900/40">
                <td className="p-5 font-bold text-slate-400 uppercase tracking-wider">{specKey}</td>
                {compareList.map(prod => (
                  <td key={prod.id} className="p-5 text-center font-semibold text-slate-700 dark:text-slate-350">
            ...
            {/* Colors */}
            <tr className="border-b border-slate-50 dark:border-slate-900/40">
              <td className="p-5 font-bold text-slate-400 uppercase tracking-wider">Colors Available</td>
              {compareList.map(prod => (
            ...
            {/* Add to cart CTA */}
            <tr>
              <td className="p-5 font-bold text-slate-400 uppercase tracking-wider">Actions</td>
              {compareList.map(prod => (
                <td key={prod.id} className="p-5 text-center">
                  <button
                    onClick={() => {
