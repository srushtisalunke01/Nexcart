...
import { 
  ...
  RefreshCw, SlidersHorizontal, Grid3X3
} from 'lucide-react';
import { PRODUCTS, CATEGORIES, BRANDS, Product } from '../data/mockData';
...
export const Home: React.FC<HomeProps> = ({ 
  onNavigate, searchFilter = '', categoryFilter = '', subcategoryFilter = '' 
}) => {
      ...
      {(activeSearch || activeCategory || activeSubcategory || activeBrand) && (
        <div className="bg-brand-50/50 dark:bg-brand-500/5 border border-brand-100 dark:border-brand-500/10 rounded-premium p-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
            ...
            <span className="font-semibold ml-2">({filteredProducts.length} items found)</span>
          </div>
          <button
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
              {BRANDS.map((br) => (
                ...
                >
                  <img src={br.logo} alt={br.name} className="max-h-full max-w-full object-contain filter grayscale dark:invert group-hover:grayscale-0 transition-all opacity-75 hover:opacity-100" />
                </motion.div>
              ))}
            </div>
          </section>
      ...
      <section className="space-y-6 pt-4">
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 rounded-premium shadow-soft">
            <p className="text-slate-400 text-base font-semibold mb-2">No matching products found.</p>
            <button onClick={clearAllFilters} className="text-xs font-bold text-brand-500 hover:text-brand-600 underline">
              Reset all filters
            </button>
          </div>
        ) : (
