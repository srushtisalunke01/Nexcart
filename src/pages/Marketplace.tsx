...
import { 
  SlidersHorizontal, AlertCircle, RefreshCw, Search, MapPin
} from 'lucide-react';
...
export const Marketplace: React.FC<MarketplaceProps> = ({ onNavigate, searchFilter = '' }) => {
  ...
  const [sellerTypeFilter, setSellerTypeFilter] = useState<'all' | 'business' | 'individual'>('all');
  const [conditionFilter, setConditionFilter] = useState<string>('all');
  const [negotiableFilter, setNegotiableFilter] = useState<boolean | 'all'>('all');
  ...
  const filteredMarketplaceListings = marketplaceProducts.filter(prod => {
    ...
    if (locationFilter && prod.location && !prod.location.toLowerCase().includes(locationFilter.toLowerCase())) {
      return false;
    }
      ...
      <div className="rounded-premium bg-gradient-to-r from-yellow-500/10 via-brand-500/10 to-orange-500/10 border border-brand-100 dark:border-brand-500/10 p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-center gap-6 shadow-soft">
        <div className="text-center sm:text-left space-y-2">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm max-w-lg">
            Discover bulk wholesale items from certified businesses, or explore used products offered directly by our luxury community.
          </p>
        </div>
        ...
        <div className="flex gap-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 px-5 py-3 rounded-2xl text-center shadow-sm">
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Wholesalers</span>
            <p className="text-xl font-display font-extrabold text-brand-500 mt-0.5">Verified</p>
          </div>
          <div className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 px-5 py-3 rounded-2xl text-center shadow-sm">
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Community</span>
            <p className="text-xl font-display font-extrabold text-indigo-500 mt-0.5">Secure</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 rounded-premium p-6 shadow-soft space-y-6">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <span className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <SlidersHorizontal className="h-4.5 w-4.5 text-brand-500" /> Filter & Search listings
          </span>
          <button
        ...
        <div className="flex flex-wrap items-center gap-6 pt-2">
          ...
          <label className="inline-flex items-center cursor-pointer">
            <select
              value={negotiableFilter === 'all' ? 'all' : negotiableFilter ? 'true' : 'false'}
              onChange={(e) => {
      ...
      <section className="space-y-6">
        ...
        {filteredMarketplaceListings.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 rounded-premium shadow-soft space-y-3">
            ...
            <p className="text-slate-400 text-sm font-semibold">No listings found matching these filters.</p>
            <button onClick={resetAllFilters} className="text-xs font-bold text-brand-500 hover:text-brand-600 underline">
              Clear filters
            </button>
          </div>
        ) : (
