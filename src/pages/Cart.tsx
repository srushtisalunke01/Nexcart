...
export const Cart: React.FC<CartProps> = ({ onNavigate }) => {
  const { 
    cart, removeFromCart, updateQuantity, appliedCoupon, 
  ...
  const totalOriginalPrice = cart.reduce((acc, item) => acc + (item.product.price * item.quantity), 0);
  ...
  const cartRecommendations = PRODUCTS.filter(p => p.trending && !cart.some(item => item.product.id === p.id)).slice(0, 3);
  ...
  if (cart.length === 0) {
    return (
      ...
      >
        ...
        <div>
          <h2 className="text-2xl font-display font-extrabold text-slate-800 dark:text-white">Your Shopping Cart is Empty</h2>
          <p className="text-slate-400 text-sm mt-1 max-w-sm">Looks like you haven't added anything to your cart yet. Explore our luxury futuristic catalogs!</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
                ...
                >
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mb-1.5">{item.product.brand}</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {item.selectedColor && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-semibold border border-slate-200/40">
                            <span style={{ backgroundColor: item.selectedColor.code }} className="h-2.5 w-2.5 rounded-full border border-white" />
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto">
                    ...
                    <div className="flex items-center border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-900 px-1 py-0.5">
                      <button 
                        onClick={() => updateQuantity(item.product.id, item.quantity - 1, item.selectedColor?.code, item.selectedSize)}
                    ...
                    {/* Delete item button */}
                    <button
                      onClick={() => removeFromCart(item.product.id, item.selectedColor?.code, item.selectedSize)}
          ...
          <div className="flex items-center gap-2 p-3 bg-slate-50 dark:bg-slate-900/20 border border-slate-100 dark:border-slate-800/30 rounded-2xl text-[10px] text-slate-400">
            ...
            <span>Orders over $150 unlock free delivery automatically. We support instant, fully-secured biometric payment verification protocols.</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="rounded-premium bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 p-6 shadow-soft space-y-4">
            ...
            {appliedCoupon ? (
              <div className="bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 p-3 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="font-extrabold text-xs text-brand-500">{appliedCoupon.code}</p>
                    <p className="text-[10px] text-slate-400">{appliedCoupon.discountPercent}% Discount Applied</p>
                  </div>
                </div>
                <button 
            ...
            {couponFeedback && (
              <p className={`text-[10px] font-bold ${couponFeedback.success ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <div className="rounded-premium bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 p-6 sm:p-8 shadow-soft space-y-6">
            ...
            <div className="space-y-3.5 text-xs text-slate-500 dark:text-slate-400">
              ...
              <div className="flex justify-between font-semibold">
                ...
                <span className={`font-bold ${shippingCharge === 0 ? 'text-green-500 bg-green-50 dark:bg-green-500/10 px-2 py-0.5 rounded' : 'text-slate-800 dark:text-white'}`}>
        ...
        <section className="space-y-6 pt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {cartRecommendations.map(prod => (
              ...
              >
                <img src={prod.images[0]} alt={prod.name} className="h-32 w-full object-contain mb-3" />
                <h4 className="font-bold text-xs text-slate-850 dark:text-white mb-2 truncate">{prod.name}</h4>
                <div className="flex justify-between items-baseline">
                  <span className="text-sm font-extrabold text-slate-900 dark:text-white font-display">${prod.discountPrice}</span>
                  <span className="text-[10px] text-brand-500 font-extrabold">Add to list</span>
                </div>
              </div>
            ))}
          </div>
