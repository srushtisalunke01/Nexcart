...
export const Profile: React.FC<ProfileProps> = ({ onNavigate, initialSection = 'dashboard' }) => {
  ...
  const loyaltyLevel = loyaltyPoints >= 500 ? 'Platinum Elite' : loyaltyPoints >= 300 ? 'Gold Star' : 'Bronze';
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 rounded-premium p-4 shadow-soft space-y-1">
          ...
          >
            <span className="flex items-center gap-2"><User className="h-4.5 w-4.5" /> Dashboard Overview</span>
          ...
          >
            <span className="flex items-center gap-2"><ShoppingBag className="h-4.5 w-4.5" /> My Orders ({orders.length})</span>
            <ChevronRight className="h-4 w-4" />
          </button>
          ...
          >
            <span className="flex items-center gap-2"><Heart className="h-4.5 w-4.5" /> Wishlist Favorites ({wishlist.length})</span>
            <ChevronRight className="h-4 w-4" />
          </button>
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'dashboard' && (
              <motion.div
                ...
                className="space-y-8"
              >
                <h2 className="text-xl font-display font-extrabold text-slate-900 dark:text-white">Profile Overview</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="p-5 rounded-premium bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/40 flex items-center justify-between shadow-sm">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Reward Cashpoints</span>
                      <p className="text-2xl font-display font-extrabold text-slate-850 dark:text-white mt-1">{loyaltyPoints} pts</p>
                    </div>
                  ...
                  <div className="p-5 rounded-premium bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800/40 flex items-center justify-between shadow-sm">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Completed Orders</span>
                      <p className="text-2xl font-display font-extrabold text-slate-850 dark:text-white mt-1">{orders.length} orders</p>
                    </div>
                ...
                <div className="bg-slate-50/50 dark:bg-slate-900/10 p-5 border border-slate-100 dark:border-slate-800 rounded-premium flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div className="flex items-center gap-3 text-left">
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase">Active Delivery Location</span>
            ...
            {activeTab === 'orders' && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-display font-extrabold text-slate-905 dark:text-white mb-4">My Orders ({orders.length})</h2>

                {orders.length === 0 ? (
                  <div className="text-center py-12 border border-slate-100 dark:border-slate-800/40 rounded-premium">
                    <p className="text-slate-400 text-sm font-semibold mb-3">No transactions found in this profile.</p>
                    <button onClick={() => onNavigate('home')} className="px-4 py-2 bg-brand-500 text-white rounded-xl text-xs font-bold shadow-md hover:bg-brand-600">
                      Shop Catalog
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {orders.map((ord) => (
                      <div key={ord.id} className="border border-slate-100 dark:border-slate-800/80 rounded-premium p-5 space-y-4 shadow-soft bg-slate-50/50 dark:bg-slate-900/10">
                        {/* Order Header info */}
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pb-3.5 border-b border-slate-100 dark:border-slate-800/60">
                          <div>
                            ...
                            <p className="font-mono text-sm font-extrabold text-slate-850 dark:text-white leading-tight">{ord.id}</p>
                          </div>
                          <div>
                            <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Order Date</span>
                            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1"><Calendar className="h-4 w-4" /> {ord.date}</p>
                          </div>
                          <div>
                          ...
                          {ord.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-center text-xs">
                              <div className="flex items-center gap-2">
                                <img src={item.product.images[0]} alt={item.product.name} className="h-10 w-10 object-contain bg-white rounded-lg p-1" />
                                <div>
                                  <p className="font-bold text-slate-800 dark:text-white max-w-[150px] sm:max-w-[280px] truncate">{item.product.name}</p>
            ...
            {activeTab === 'wishlist' && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-display font-extrabold text-slate-905 dark:text-white mb-4">Saved Favorites ({wishlist.length})</h2>

                {wishlist.length === 0 ? (
                  <div className="text-center py-12 border border-slate-100 dark:border-slate-800/40 rounded-premium">
                    <p className="text-slate-450 text-sm font-semibold mb-3">Your wishlist is empty.</p>
                    <button onClick={() => onNavigate('home')} className="px-4 py-2 bg-brand-500 text-white rounded-xl text-xs font-bold shadow-md">
                      Go Browse
                    </button>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {wishlist.map((prod) => (
                      <div key={prod.id} className="border border-slate-100 dark:border-slate-800/80 rounded-premium p-4 flex flex-col justify-between gap-4 shadow-soft bg-slate-50/50 dark:bg-slate-900/10">
                        <div className="flex items-center gap-3">
                          <img src={prod.images[0]} alt={prod.name} className="h-16 w-16 object-contain bg-white rounded-xl p-1.5" />
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            ...
                            <span>{addedItem === prod.id ? "Added!" : "Move Cart"}</span>
                          </button>
                          <button
            ...
            {activeTab === 'addresses' && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-display font-extrabold text-slate-905 dark:text-white mb-4">Saved Locations</h2>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {addresses.map(addr => (
                    <div key={addr.id} className="p-5 rounded-premium border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/10 shadow-soft">
                      <div className="flex justify-between items-center mb-3">
                        <span className="font-extrabold text-sm text-slate-850 dark:text-white">{addr.name}</span>
                        {addr.isDefault && (
                      ...
                      <p className="text-[10px] text-slate-450 font-bold mt-4">{addr.phone}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            {activeTab === 'wallet' && (
              <motion.div
                ...
                className="space-y-8"
              >
                <h2 className="text-xl font-display font-extrabold text-slate-905 dark:text-white">NexWallet & Loyalty Programs</h2>
                ...
                <div className="border border-slate-100 dark:border-slate-800 rounded-premium p-6 space-y-4 shadow-soft">
                  <div className="flex justify-between items-center">
                    <div>
                      <h4 className="font-extrabold text-sm text-slate-800 dark:text-white">Loyalty Cashpoint Program</h4>
                      <p className="text-[10px] text-slate-400">Unlock luxury discount tokens at 500 points.</p>
                    </div>
                    <span className="text-sm font-extrabold text-slate-900 dark:text-white">{loyaltyPoints} / 500 pts</span>
                  </div>
