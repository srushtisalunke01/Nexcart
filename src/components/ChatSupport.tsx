  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (userQuery.includes("shipping") || userQuery.includes("delivery") || userQuery.includes("arrive")) {
        ...
        reply = "NexCart offers a premium 30-day return policy. If you are unsatisfied, click 'Return Item' on the Order page to request a free return parcel. We arrange home pick-ups and refund instantly.";
      } else if (userQuery.includes("hello") || userQuery.includes("hi")) {
        reply = "Hello! How is your shopping experience going? Feel free to ask about our luxury watches, sneakers, or ordering flow.";
      }
          ...
          >
            ...
            <div className="p-4 bg-gradient-to-r from-cyber-cyan via-brand-500 to-cyber-gold text-white flex items-center justify-between border-b border-cyber-cyan/15">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-display font-bold text-sm">NexCart Support</h4>
                ...
                >
                  <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 shadow-sm text-sm ${
                    ...
                    <span className={`block text-[9px] text-right mt-1 ${
                      msg.sender === 'user' ? 'text-brand-100' : 'text-slate-400 dark:text-slate-500'
                    }`}>
                      {msg.time}
                    </span>
                  </div>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-white dark:bg-premium-cardLight/5 border border-slate-100 dark:border-slate-800 rounded-2xl rounded-bl-none px-4 py-3 text-sm flex gap-1 items-center">
                    <span className="h-2 w-2 rounded-full bg-slate-400 dark:bg-slate-600 animate-bounce" style={{ animationDelay: '0s' }} />
                    <span className="h-2 w-2 rounded-full bg-slate-400 dark:bg-slate-600 animate-bounce" style={{ animationDelay: '0.15s' }} />
                    <span className="h-2 w-2 rounded-full bg-slate-400 dark:bg-slate-600 animate-bounce" style={{ animationDelay: '0.3s' }} />
                  </div>
                </div>
              )}
              <div ref={chatEndRef} />
            ...
            {/* Input area */}
            <form onSubmit={handleSend} className="p-3 border-t border-slate-100 dark:border-slate-800 flex gap-2 bg-white dark:bg-premium-cardDark">
              <input
                type="text"
