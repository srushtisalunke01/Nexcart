...
const SAMPLE_IMAGES = [
  { id: "img-1", label: "Orange Running Shoes", url: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=150&auto=format&fit=crop&q=60" },
  { id: "img-2", label: "Studio Headphones", url: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=150&auto=format&fit=crop&q=60" },
  { id: "img-3", label: "Chronograph Watch", url: "https://images.unsplash.com/photo-1547996160-81dfa63595aa?w=150&auto=format&fit=crop&q=60" }
];

export const ImageSearchModal: React.FC<ImageSearchModalProps> = ({ isOpen, onClose, onSearch }) => {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        >
          ...
          {stage === 'idle' && (
            <div>
              ...
              <div className="border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-8 mb-6 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 transition-colors">
                ...
                <h4 className="font-semibold text-slate-700 dark:text-slate-200 mb-1">Upload an image file</h4>
                <p className="text-xs text-slate-400 mb-4">PNG, JPG, or WEBP up to 5MB</p>
              ...
              <div className="text-left">
                <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Try these demo templates:</h5>
                  ...
                  {SAMPLE_IMAGES.map((img) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/45 flex items-end p-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <span className="text-[10px] text-white leading-tight font-medium">{img.label}</span>
                      </div>
                    </button>
                  ))}
                </div>
          ...
          {(stage === 'uploading' || stage === 'analyzing' || stage === 'complete') && (
            <div className="flex flex-col items-center">
              ...
              <div className="relative h-48 w-48 rounded-xl overflow-hidden mb-6 border border-slate-100 dark:border-slate-800 shadow-md">
                <img src={selectedImg || ''} alt="Analysis Preview" className="h-full w-full object-cover" />
              ...
              <div className="w-full max-w-xs h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-4">
                <motion.div
                  initial={{ width: "0%" }}
                  animate={{ 
                    width: stage === 'uploading' ? "35%" : stage === 'analyzing' ? "85%" : "100%" 
                  }}
