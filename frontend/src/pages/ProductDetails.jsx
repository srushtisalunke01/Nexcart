...
export const ProductDetails = ({ productId, onNavigate }) => {
  ...
  const handleContactSeller = () => {
    ...
    addNotification(
      "Chat Initialized",
      `Conversation active with ${product.sellerName}`,
    );
  };
  ...
  const handleQuoteSubmit = (e) => {
    ...
    addNotification(
      "Quote Request Sent",
      `Quotation sent successfully to ${product.sellerName}`,
    );
    setTimeout(() => {
      setQuoteSuccess(false);
      setCompanyName("");
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="flex gap-3 items-center">
                  {product.sellerLogo ? (
                    <img
                      ...
                      alt="seller logo"
                      className="h-12 w-12 rounded-xl object-contain border p-1 bg-white"
                    />
                  ) : (
            ...
            <div className="mb-6 p-4 rounded-xl border border-dashed border-brand-300 dark:border-brand-500/20 bg-brand-50/5 dark:bg-brand-500/5 space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                {product.priceTiers?.map((t, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between text-xs font-semibold py-1 border-b border-slate-100 dark:border-slate-800 last:border-b-0"
                  >
        ...
        <div className="space-y-3.5">
          ...
          >
            ...
            />
            <span>
              {isFavorited ? "Saved in Wishlist" : "Add to Wishlist Favorites"}
            </span>
          </button>
          ...
          {product.sellerType === "business" && (
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800/80">
              <span className="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                Or Request wholesale quotation
              </span>
              {quoteSuccess ? (
                ...
                <form onSubmit={handleQuoteSubmit} className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[9px] text-slate-400 mb-1 font-bold">
                        Company Name
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setQuoteQty(
                            parseInt(e.target.value) || product.moq || 5,
                          )
                        }
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-brand-500 dark:text-white"
                      />
                  ...
                  <div>
                    <label className="block text-[9px] text-slate-400 mb-1 font-bold">
                      Negotiation Notes / Custom Terms
                    </label>
                    <input
                      type="text"
          ...
          {product.sellerType === "individual" && (
            ...
            <div className="space-y-3 pt-4 border-t border-slate-105 dark:border-slate-800/80">
              <span className="block text-xs font-bold text-slate-400 tracking-wider mb-2">
                Used Marketplace Listing ({product.condition} condition)
              </span>
              <div className="flex gap-3">
                <button
                  onClick={handleContactSeller}
      ...
      <div className="rounded-premium bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 shadow-soft overflow-hidden">
        <div className="flex border-b border-slate-100 dark:border-slate-800">
          {["specs", "features", "reviews", "questions"].map((tab) => (
            <button
              key={tab}
          ...
          {activeTab === "questions" && (
            <div className="space-y-6">
              ...
              <div className="space-y-4 divide-y divide-slate-100 dark:divide-slate-800 text-left">
                {qaList.length === 0 ? (
                  <p className="text-slate-450 text-xs italic text-center py-4">
                  ...
                  qaList.map((qa, i) => (
                    <div key={i} className="pt-4 space-y-1.5 text-xs">
                      <p className="font-bold text-slate-800 dark:text-white flex items-center gap-1">
                        <span className="bg-brand-500/10 text-brand-600 px-1.5 py-0.5 rounded text-[9px]">
                          Q
                        </span>
                        "{qa.q}"{" "}
                      ...
                      {qa.a ? (
                        <p className="pl-6 text-slate-600 dark:text-slate-400 flex items-center gap-1">
                          <span className="bg-green-500/10 text-green-600 px-1.5 py-0.5 rounded text-[9px]">
                            A
                          </span>
                          "{qa.a}"
                        </p>
                      ) : (
                        <p className="pl-6 text-slate-450 italic">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {similarProducts.map((p) => (
              ...
              >
                ...
                />
                <h4 className="font-bold text-xs text-slate-800 dark:text-white mb-2 leading-tight line-clamp-2 h-8">
                  {p.name}
                </h4>
