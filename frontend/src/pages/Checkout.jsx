    ...
    return (
      <div className="text-center py-20">
        <p className="text-slate-400 text-sm font-semibold mb-4">
          No active checkout checkout parameters detected.
        </p>
        <button
          onClick={() => onNavigate("home")}
          className="px-5 py-2.5 bg-brand-500 text-white rounded-xl text-xs font-bold shadow-md"
        >
          Go Home
        </button>
      </div>
    ...
    >
      ...
      {checkoutStep !== "success" && (
        <div className="max-w-2xl mx-auto flex items-center justify-between relative px-6">
          ...
          >
            <div
              className={`h-9 w-9 rounded-full flex items-center justify-center font-bold text-xs shadow-md border ${
          ...
          >
            <div
              className={`h-9 w-9 rounded-full flex items-center justify-center font-bold text-xs shadow-md border transition-colors ${
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="rounded-premium bg-white dark:bg-premium-cardDark border border-slate-100 dark:border-slate-800/40 p-6 sm:p-8 shadow-soft">
              <h2 className="text-xl font-display font-extrabold text-slate-905 dark:text-white mb-6 flex items-center gap-2">
                <MapPin className="h-5 w-5 text-brand-500" /> Select Shipping
                Location
              </h2>
              ...
              {!showAddAddrForm ? (
                ...
                >
                  <h3 className="font-bold text-sm text-slate-850 dark:text-white mb-2">
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-100 dark:border-slate-800/30">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-center">
                      <div className="bg-white dark:bg-slate-950 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col items-center">
                        ...
                        <div className="h-32 w-32 bg-slate-100 rounded-lg flex items-center justify-center border border-slate-200">
                          <Compass
                            className="h-16 w-16 text-slate-300 animate-spin"
                            style={{ animationDuration: "6s" }}
                          />
                        </div>
                        <span className="text-[9px] font-bold text-slate-400 mt-2">
                          SCAN DYNAMIC MERCH QR
                        </span>
                      </div>
                      <div className="flex-1 w-full space-y-3">
                        <p className="text-xs text-slate-400">
                          Scan the merchant QR code with any BHIM, GooglePay,
                          PayTM, or PhonePe application, or enter your UPI
                          Address below.
                        </p>
                        <input
                          type="text"
                ...
                {paymentMethod === "Wallet" && (
                  <div className="space-y-3">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                      NexWallet Account Verification
                    </h3>
                    <div className="flex justify-between items-center bg-white dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      ...
                      <div>
                        ...
                        <p className="text-xl font-display font-extrabold text-brand-500">
                          ₹{total}
                        </p>
                      </div>
                    </div>
                    {walletBalance < total && (
                      <p className="text-xs font-bold text-red-500 animate-pulse">
                        Insufficient balance in your digital wallet. Please
                ...
                {paymentMethod === "COD" && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-400">
                      Paying by Cash/Card at delivery. Please prepare exact
                      change for shipping couriers. Delivery requires signing
                      biometric verification protocols on terminal devices.
                    </p>
                  </div>
                )}
              </div>
        ...
        >
          <div className="flex flex-col items-center">
            ...
            <p className="text-slate-400 text-sm mt-1">
              Thank you! Your transaction cleared successfully.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-900/50 p-5 rounded-2xl text-left border border-slate-100 dark:border-slate-800/30 text-xs">
            <div>
              <p className="text-slate-450 font-bold uppercase tracking-wider mb-1">
                Transaction Invoice:
              </p>
              <p className="font-mono font-extrabold text-sm text-slate-800 dark:text-white select-all cursor-pointer flex items-center gap-1">
                ...
                <Copy className="h-3 w-3 text-slate-400 hover:text-brand-500" />
              </p>
            </div>
            <div>
              <p className="text-slate-450 font-bold uppercase tracking-wider mb-1">
            ...
            <div className="sm:col-span-2 pt-3.5 border-t border-slate-200 dark:border-slate-800/60">
              <p className="text-slate-455 font-bold uppercase tracking-wider mb-1 text-[10px]">
                Deliver Address:
              </p>
              <p className="font-bold text-slate-700 dark:text-slate-350">
                {placedOrderDetails.address.name}
              </p>
              <p className="text-slate-400">
                {placedOrderDetails.address.street},{" "}
                {placedOrderDetails.address.city},{" "}
                {placedOrderDetails.address.state} -{" "}
                {placedOrderDetails.address.zipCode}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center text-left">
              <div>
                <h3 className="font-display font-extrabold text-sm text-slate-800 dark:text-white flex items-center gap-1.5">
                  <Compass
                    className="h-4.5 w-4.5 text-brand-500 animate-spin"
                    style={{ animationDuration: "8s" }}
                  />{" "}
                  Live Order Tracking
                </h3>
                <p className="text-[10px] text-slate-400">
                  Dynamic logistics telemetry updating in real-time.
                </p>
              </div>
              ...
              <div className="flex justify-between">
                {[0, 1, 2, 3].map((step) => (
                  <div key={step} className="flex flex-col items-center">
                    <div
                      className={`h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs shadow border transition-all ${
                        shippingStep >= step
            ...
            <div className="h-52 w-full bg-slate-900 rounded-2xl relative overflow-hidden border border-slate-800 shadow-inner flex items-center justify-center">
              ...
              {/* Route lines */}
              <svg
                className="absolute inset-0 w-full h-full text-brand-500/35"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M 50 160 Q 150 60 280 120 T 600 80"
                  stroke="#FF6B00"
                  strokeWidth="4"
                  fill="none"
                  strokeDasharray="6,6"
                  className="animate-[shimmer_15s_infinite_linear]"
                />
              </svg>
              ...
              <div className="absolute left-[50px] bottom-[30px] flex flex-col items-center">
                ...
                <span className="text-[9px] font-bold text-white bg-slate-950/80 px-2 py-0.5 rounded border border-slate-800">
                  Dispatch Hub
                </span>
              </div>
