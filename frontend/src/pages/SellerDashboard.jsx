...
export const SellerDashboard = ({ onNavigate }) => {
  ...
  const myListings = marketplaceProducts.filter(
  ...
  const handleDelete = (id, name) => {
    if (confirm(`Are you sure you want to delete "${name}"?`)) {
      ...
      addNotification(
        "Listing Deleted",
        `"${name}" has been permanently removed from your dashboard.`,
      );
    }
  };

  const handleMarkSold = (id, name) => {
    markAsSold(id);
    addNotification(
      "Listing Sold",
      `"${name}" was marked as sold. Congratulations on your sale!`,
    );
  };
  ...
  const handleRenew = (id, name) => {
    ...
    addNotification(
      "Listing Renewed",
      `"${name}" has been successfully renewed and bumped up in results.`,
    );
  };
            ...
            <p className="text-2xl font-display font-extrabold text-slate-850 dark:text-white mt-1">
              {
                chats.filter(
                  (c) =>
                    c.sellerName === "Alex Stark" ||
                    c.sellerType === "individual",
                ).length
              }
            </p>
          </div>
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start gap-2">
                              <h3 className="font-bold text-xs sm:text-sm truncate pr-2">
                                {prod.name}
                              </h3>
                              <span
                          ...
                          <div className="flex gap-2 mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-850">
                            {!prod.isSold ? (
                              <button
                                onClick={() =>
                                  handleMarkSold(prod.id, prod.name)
                                }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-405 mt-0.5">
                        Your listings are currently receiving higher engagement
                        than 82% of neighborhood sellers.
                      </p>
                    </div>
                  </div>
                </div>
