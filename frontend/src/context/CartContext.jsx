...
const DEFAULT_NOTIFICATIONS = [
  {
    ...
    message:
      "Enjoy a 10% discount on your first order. Use code NEXCART10 at checkout.",
    date: "Just now",
    read: false,
  },
  {
    ...
    message:
      "Get up to 25% off on high-end headphones. Limited inventory remains.",
    date: "2 hours ago",
    read: false,
  },
];
...
export const CartProvider = ({ children }) => {
  ...
  const [walletBalance, setWalletBalance] = useState(180.0);
  ...
  const removeFromCart = (productId, colorCode, size) => {
    setCart((prev) =>
      prev.filter(
        (item) =>
          !(
  ...
  const updateQuantity = (productId, quantity, colorCode, size) => {
    if (quantity <= 0) {
  ...
  const addToCompare = (product) => {
    ...
    if (status) {
      addNotification(
        "Added to Compare",
        `${product.name} added to comparison dashboard.`,
      );
    }
    return status;
  };
  ...
  const placeOrder = (paymentMethod) => {
    ...
    addNotification(
      ...
      `Thank you! Your order ${newOrder.id} is being processed.`,
    );
    return newOrder;
  };
