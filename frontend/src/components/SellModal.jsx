...
export const SellModal = ({ isOpen, onClose }) => {
  ...
  const handleAddTier = () => {
    ...
    if (!isNaN(qty) && !isNaN(prc)) {
      setTiers((prev) =>
        [...prev, { qty, price: prc }].sort((a, b) => a.qty - b.qty),
      );
      setNewTierQty("");
      setNewTierPrice("");
    }
  ...
  const handleRegisterAndSubmit = (e) => {
    e.preventDefault();
    if (!isRegistered) {
      registerBusiness({
        companyName,
    ...
    createListing({
      ...
      category,
      parentCategory:
        CATEGORIES.find((c) => c.subcategories.some((s) => s.id === category))
          ?.id || "electronics",
      images:
        images.length > 0
          ? images
            ...
            {successMsg ? (
              ...
              >
                ...
                <p className="text-xs text-slate-400">
                  Your item is now active on the NEXUS Marketplace catalog.
                </p>
              </motion.div>
            ) : (
              <div className="space-y-6">
                ...
                {sellerType === "individual" ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm focus:outline-none dark:text-white"
                        >
                          {CATEGORIES.flatMap((cat) => cat.subcategories).map(
                            (sub) => (
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        <label className="inline-flex items-center cursor-pointer mt-1">
                          ...
                          <span className="ml-2 text-xs font-semibold text-slate-600 dark:text-slate-400">
                            {negotiable ? "Accept Offers" : "Firm Price"}
                          </span>
                        </label>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3 mt-3">
                        {images.map((img, idx) => (
                          <div
                            key={idx}
                            className="relative h-16 w-16 border rounded-xl overflow-hidden group"
                          >
                            <img
                              src={img}
                              alt="listing"
                  ...
                  >
                    ...
                    {!isRegistered && (
                      <div className="border border-brand-100 dark:border-brand-500/10 p-5 rounded-2xl bg-brand-50/20 dark:bg-brand-500/5 space-y-4">
                        <span className="text-xs font-bold text-brand-500 flex items-center gap-1.5 uppercase">
                          <Building className="h-4 w-4" /> Business Registration
                          Required
                        </span>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                          ...
                          <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">
                              Tax ID / Business Registration ID
                            </label>
                            <input
                              type="text"
                              required
                    ...
                    <div className="border border-slate-100 dark:border-slate-800 rounded-2xl p-4 space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tiers.map((t, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-900 px-3 py-1.5 rounded-xl border text-xs font-semibold"
                          >
                            <span>
                              Qty {t.qty}+: ${t.price}
                            </span>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-400 uppercase mb-2">
                        Wholesale Specifications / Notes
                      </label>
                      <textarea
                        required
                        rows={2}
